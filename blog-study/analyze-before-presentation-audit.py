...
def make_analysis():
    ...
    for m,rs in groups.items():
        ...
        stats[m]={'name':NAMES[m],'test_n':len(rs),'correct':int(ok.sum()),'accuracy':float(ok.mean()),'accuracy_ci95':np.quantile(ok[samples].mean(axis=1),[.025,.975]).tolist(),'paired_accuracy_difference_vs_jev':float((ok-base).mean()),'paired_difference_ci95':np.quantile((ok-base)[samples].mean(axis=1),[.025,.975]).tolist(),'macro_f1':macro_f1(rs,labels),'failures':len(rs)-len(valid),'median_ms':nearest(lat,.5),'p95_ms':nearest(lat,.95),'cost_per_1000_usd':sum(costs)/len(rs)*1000,'cost_basis':'provider billed' if all(x is not None for x in billedtest) else 'billed where available, otherwise usage estimate/reservation','all_phase_accounted_usd':sum(allcost),'all_phase_calls':len(modelrows),'all_phase_billed_usd':sum(billed) if all(x is not None for x in billed) else None,'brier':float(np.mean((prob-correct)**2)),'probability_error_detection':discrimination(correct,prob),'timing_repeat_median_ms':nearest(repeats,.5),'timing_repeat_p95_ms':nearest(repeats,.95)}
        ...
        for field,label in [('probability','probability'),*([('nativeConfidence','native')] if m=='jev' else [])]:
            scores=np.array([r.get(field,float('nan')) for r in rs]);aucs=[]
            for ix in samples:
    ...
    lines=['# Fast-model BANKING77 follow-up','', '500 unique held-out English messages across 77 intents. One prediction per model; 50 timing repeats excluded from accuracy.', '', '| Model | Correct | Accuracy | Median ms | p95 ms | USD / 1,000 | Error AUROC | Brier |','|---|---:|---:|---:|---:|---:|---:|---:|']
    for m,s in stats.items():lines.append(f"| {s['name']} | {s['correct']}/500 | {100*s['accuracy']:.1f}% | {s['median_ms']:.0f} | {s['p95_ms']:.0f} | {s['cost_per_1000_usd']:.3f} | {s['probability_error_detection']['auroc']:.3f} | {s['brier']:.4f} |")
...
def plots(result,groups):
    ...
    fig,axs=plt.subplots(1,3,figsize=(15,7));fig.subplots_adjust(top=.76,bottom=.27,left=.07,right=.98,wspace=.45)
    ...
    titles=['Correct intent (%) ↑','Typical complete answer (s) ↓','Cost / 1,000 requests (USD) ↓']
    ...
    fig.text(.065,.13,'Latency includes network and schema validation. One outstanding call per provider, four providers in parallel.\nCosts use reported charges where available; otherwise usage-based estimates. Timing repeats and development excluded.\nOne dataset, one host, one run. This is a blog experiment, not a universal model ranking.',fontsize=10.5,linespacing=1.6,color='#555555')
    save(fig,'01-results')
    ...
    for ax,m in zip(axs.ravel(),MODELS):
        ...
        baseline=100*(1-result['stats'][m]['accuracy']);ax.axhline(baseline,color='#999999',ls=':',linewidth=1)
    ...
    fig.text(.085,.065,'Dots: frozen thresholds selected on 200 separate messages. Dashed horizontal line: 5% error target.\nDotted line: expected error under random deferral. Curves are descriptive, not used to choose test thresholds.',fontsize=10.5,color='#555555',linespacing=1.5)
